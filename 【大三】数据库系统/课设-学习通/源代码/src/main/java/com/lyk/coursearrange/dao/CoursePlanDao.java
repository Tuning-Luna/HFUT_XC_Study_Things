...
public interface CoursePlanDao extends BaseMapper<CoursePlan> {
    ...
    // 将上课计划插入上课计划的表中
//    @Insert("insert into course_plan ")
//    void insertCoursePlan(CoursePlan coursePlan);

    // 插入课程计划

    @Insert("insert into course_plan(grade_no, class_no, course_no, teacher_no, classroom_no, class_time, semester) values(#{grade_no}, #{class_no}, #{course_no}, #{teacher_no}, #{classroom_no}, #{class_time}, #{semester})")
