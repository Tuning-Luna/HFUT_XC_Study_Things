...
void CExperiment_Frame_OneView::OnDraw(CDC* pDC)
{
	...
	case 1: //Mid_Bresenham
		...
		break;
	case 2: //同时绘制 DDA 和 Bresenham，稍微错开并不同颜色
	{
		// 保存原颜色
		COLORREF originalColor = m_PointColor;
		...
		// 绘制 Bresenham
		m_PointColor = RGB(250, 80, 123);
		Mid_Bresenham(X0 + 1, Y0, X1 + 1, Y1);

		// 恢复原颜色
		m_PointColor = originalColor;
	}
	break;
...
void CExperiment_Frame_OneView::DDA(int X0, int Y0, int X1, int Y1)
{
	...
	{
		DrawPixel((int)(x + 0.5f), (int)(y + 0.5f));
		x += xInc;
		y += yInc;
	}
}
