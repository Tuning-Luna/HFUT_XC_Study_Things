	...
	double Kd)
{
	...
	double vertexIntensities[4];
	for (int i = 0; i < 4; ++i) {
		double Lx = lightPos[0] - Vertices[i][0];
		double Ly = lightPos[1] - Vertices[i][1];
		double Lz = lightPos[2] - Vertices[i][2];

		double L_len = sqrt(Lx * Lx + Ly * Ly + Lz * Lz);
		if (L_len == 0) {
			vertexIntensities[i] = 0.0;
			continue;
		}
		...
		vertexIntensities[i] = Ip * Kd * dot;
	}

	I = uv[0] * vertexIntensities[0] +
		uv[1] * vertexIntensities[1] +
		uv[2] * vertexIntensities[2] +
		uv[3] * vertexIntensities[3];
	...
	double Kd)
{
	...
	double Nx = uv[0] * Normals[0][0] + uv[1] * Normals[1][0] +
		uv[2] * Normals[2][0] + uv[3] * Normals[3][0];
	double Ny = uv[0] * Normals[0][1] + uv[1] * Normals[1][1] +
		uv[2] * Normals[2][1] + uv[3] * Normals[3][1];
	double Nz = uv[0] * Normals[0][2] + uv[1] * Normals[1][2] +
		uv[2] * Normals[2][2] + uv[3] * Normals[3][2];

	double N_len = sqrt(Nx * Nx + Ny * Ny + Nz * Nz);
	if (N_len == 0) return 0.0;
	Nx /= N_len; Ny /= N_len; Nz /= N_len;


	double Lx = lightPos[0] - Pt[0];
	double Ly = lightPos[1] - Pt[1];
	double Lz = lightPos[2] - Pt[2];
